...
- {gh-issue}`278` {gh-pr}`280` **BREAKING CHANGE**: the `Bus` constructor now accepts a nominal voltage
  (phase-to-phase, in V), a `min_voltage_level` (without unit) and a `max_voltage_level` (without unit). These
  arguments replace `min_voltage` and `max_voltage`. The `Bus` class now have some additional properties:
