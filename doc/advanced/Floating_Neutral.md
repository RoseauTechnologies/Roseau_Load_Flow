...
difference between the voltages of phases remains normal, but the 0V reference of the neutral no
longer exists. This destroys the balance of phase-to-neutral voltages exposing some appliances to
severe overvoltage (up to the phase-to-phase voltage) or undervoltage (down to 0V) conditions. The
larger the load imbalance, the more serious the issue becomes.
...
When defining a load or source with a neutral connection (e.g., a 3-phase load with phases "abcn"),
on a bus without a neutral connection (e.g., a 3-phase bus with phases "abc"), the neutral is
implicitly floating. This is because the bus doesn't have a neutral to connect to.
...
connection. This is because the neutral of a load or source is automatically considered floating
when the bus doesn't have a neutral.

The plots of the voltage phasors above show how the potential at the neutral point moves away from
the center of the phasor diagram when the neutral is floating. For the balanced load, the neutral
remains at the center of the phasor diagram, while for the unbalanced load, the neutral moves away
from the center towards the phase with the lowest impedance. The phase with the highest impedance
(i.e with the lowest power) is the one exposed to the highest voltage.

### Explicit floating neutral

You can also explicitly define a floating neutral by passing `connect_neutral=False` when creating
a load or source. This is useful when you want to model both elements with floating and connected
neutrals on the same bus.
