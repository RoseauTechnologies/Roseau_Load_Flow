...
\begin{document}
    \begin{tikzpicture}[%
        ...
        \begin{scope}[local bounding box=dessin]
            ...
            \path[fill=RoseauGrey, draw=black] (neutral center) circle[radius=1];%
            \draw[pattern=north east lines, pattern color=red, preaction={fill, white}] (neutral center) circle[radius=0.90];%
            \node[shape=circle, draw, fill=white] (neutral center node) at (neutral center) {$\nrm$};%

            % Phases
            \foreach \x/\y/\z in {a/135/$\arm$,b/45/$\brm$,c/-90/$\crm$} {%
                \path (neutral center) ++(\y:2.5) coordinate (\x center);%
                \path[fill=RoseauGrey, draw=black] (\x center) circle[radius=1.5];%
                \draw[pattern=north east lines, pattern color=RoseauGreen, preaction={fill, white}] %
                (\x center) circle[radius=1.4];%
                \node[shape=circle, draw, fill=white] at (\x center) {\z};%
            }%
            \draw[dashed] (neutral center) circle[radius=4];%

            % Hauteur
            \coordinate (top height) at (neutral center -| {(-4.5,0)});%
            \draw[{Latex[]-Latex[]}] (-4.5,0.5) -- (top height) node[midway,right] {$h$};
            \draw[dashed] (top height) -- (neutral center node);%
        \end{scope}
