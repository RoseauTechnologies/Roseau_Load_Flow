...
\begin{tikzpicture}[%
  ...
  % Lignes
  \draw (A) to[generic, v^<=$\underline{V_{\arm}} - \underline{V_{\brm}}$,
    i_ =$\underline{I_{\abrm}}$, *-*] (B) %
    to[generic, v^<=$\underline{V_{\brm}} - \underline{V_{\crm}}$, i_ =$\underline{I_{\bcrm}}$, *-*]
    (C) %
    to[generic, v^<=$\underline{V_{\crm}} - \underline{V_{\arm}}$, i_ =$\underline{I_{\carm}}$, *-*]
    (A);%
