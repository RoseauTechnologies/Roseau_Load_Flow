...
\begin{tikzpicture}[%
  ...
  \tikzset{ transformer/.pic={ \draw (-0.2,0) circle[radius=0.4] (0.2,0) circle[radius=0.4]; },}
  ...
  % Câbles principaux
  % A
  \draw (\xl,\ya) to[short,*-*,i=$\underline{I_{1,\arm}}$] (\xy,\ya) to[short]
    (\xtransformerm,\ya) to[short] node[near start,above=0.25cm,circ] {} (T);%
  \draw (\xtransformerp,\ya) to[short] node[near start,above=0.25cm,circ] {} (T);%
  \draw (\xtransformerp,\ya) -- (\xrt,\ya) to[generic, l=$\dfrac{\underline{Z_2}}{2}$,
    label distance=6pt, -] (\xz,\ya) to[short,-*,i<=$\underline{I_{2,\arm}}$] (\xm,\ya);%

  % B
  \draw (\xl,\yb) to[short,*-*,i=$\underline{I_{1,\brm}}$] (\xy,\yb) to[short]
    (\xtransformerm,\yb) to[short] (T);%
  ...
  % Neutre
  \draw (T.east) to[short] (\xtransformerp,\yn)
    -- (\xz,\yn) to[short,-*,i<=$\underline{I_{2,\nrm}}$] (\xm,\yn);%

  % Ym
  \draw (\xy,\ya) to[generic, l=$\underline{Y_{\mathrm{m}}}$, v<=$\underline{U_{1,\abrm}}$, -]
    (\xy,\yb);%
