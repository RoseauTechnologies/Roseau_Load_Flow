...
Run with roseau-load-flow==0.12.0, NOT WITH THE CURRENT VERSION::
    ...
    uv run --no-project --script roseau/load_flow_single/io/tests/data/generators/v4.py
"""
...
from roseau.load_flow_single.io.dict import NETWORK_JSON_VERSION as VER

if VER != 4:
    raise ValueError(
        f"This script is meant to generate version 4 JSON files, current version: {VER}"
        f"\nExecute with `uv run --no-project --script roseau/load_flow_single/io/tests/data/generators/v4.py`"
    )

OUTPUT = Path(__file__).parent.parent / f"network_json_v{VER}.json"
