...
Run with roseau-load-flow==0.1.0, NOT WITH THE CURRENT VERSION::

    uv run --no-project --script roseau/load_flow_single/io/tests/data/generators/v3.py
"""
...
# dependencies = [
#     "roseau-load-flow-single==0.1.0",
# ]
# ///
from pathlib import Path

import roseau.load_flow_single as rlfs
from roseau.load_flow_single.io.dict import NETWORK_JSON_VERSION

if NETWORK_JSON_VERSION != 3:
    raise ValueError(
        f"This script is meant to generate version 3 JSON files, current version: {NETWORK_JSON_VERSION}"
        f"\nExecute with `uv run --no-project --script roseau/load_flow_single/io/tests/data/generators/v3.py`"
    )
...
transformer0 = rlfs.Transformer(id="transformer0", bus1=bus1, bus2=bus2, parameters=tp0, tap=1.025, max_loading=1.1)
