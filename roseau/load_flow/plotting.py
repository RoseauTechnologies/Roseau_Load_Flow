...
from roseau.load_flow.typing import ComplexArray, ComplexLike, FloatLike
...
# Utility functions
#
def _get_rot(vector: ComplexLike) -> float:
...
def _get_align(rot: FloatLike) -> tuple[str, str]:
...
def _draw_voltage_phasor(ax: "Axes", potential1: ComplexLike, potential2: ComplexLike, color: str) -> None:
...
def plot_interactive_map(
    ...
    *,
    ...
    buses_gdf = network.buses_frame.reset_index()
    buses_gdf["element_type"] = "bus"
    lines_gdf = network.lines_frame.reset_index()
    ...
    geom_union = buses_gdf.unary_union
