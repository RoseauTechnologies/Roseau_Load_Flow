...
from roseau.load_flow import CurrentLoad, ImpedanceLoad, Switch, Transformer
...
from roseau.load_flow.single.lines import Line
...
class ElectricalNetwork(JsonMixin, CatalogueMixin[JsonDict]):
    ...
    def __init__(
        ...
        *,
        ...
        for bus in self.buses.values():
            next(iter(self.grounds.values())).connect(bus)
        self._check_validity(constructed=False)
        self._create_network()
        self._valid = True
        self._solver = AbstractSolver.from_dict(data={"name": "backward_forward", "params": {}}, network=self)
        if crs is None:
            crs = "EPSG:4326"
    ...
    @classmethod
    def from_element(cls, initial_bus: Bus, ground: Ground) -> Self:
        ...
        elements: list[Element] = [initial_bus, ground]
    ...
    def grounds_frame(self) -> pd.DataFrame:
        ...
        return pd.DataFrame.from_records(
            data=[(ground.id, bus_id) for ground in self.grounds.values() for bus_id in ground.connected_buses],
            columns=["id", "bus_id"],
            index=["id", "bus_id"],
        )
    ...
    def potential_refs_frame(self) -> pd.DataFrame:
        ...
        return pd.DataFrame.from_records(
            data=[(pref.id, pref.element.id) for pref in self.potential_refs.values()],
            columns=["id", "element_id"],
            index="id",
        )
    ...
    def _propagate_potentials(self) -> None:
        ...
        while elements:
            ...
            if not isinstance(element, Ground):
                ...
                for e in element._connected_elements:
                    if e not in visited and isinstance(e, PotentialRef):  # Do not go from ground to buses/branches
                        elements.append((e, potential))

        if len(visited) < len(self.buses) + len(self.lines) + len(self.transformers) + len(self.switches):  # TODO all e
