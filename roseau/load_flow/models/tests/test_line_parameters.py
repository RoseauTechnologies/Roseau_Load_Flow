...
def test_get_catalogue():
    ...
    for field_name, value, expected_size in (
        ...
        ("section", 150, 13),
        ("section_neutral", 150, 10),
        ("section", Q_(1.5, "cm²"), 13),
        ("section_neutral", Q_(1.5, "cm²"), 10),
    ):
        filtered_catalogue = LineParameters.get_catalogue(**{field_name: value})
        assert filtered_catalogue.shape == (expected_size, 15), f"Failed for {field_name}={value}"
