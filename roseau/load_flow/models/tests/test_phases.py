...
def test_transformer_three_phases():
    ...
    tp = TransformerParameters.from_open_and_short_circuit_tests(
        id="H61_50kVA", type="Dyn11", up=20000, us=400, sn=50 * 1e3, p0=145, i0=1.8 / 100, psc=1350, vsc=4 / 100
    )
    for ph in ("ba", "nc", "anb", "nabc", "acb"):
        with pytest.raises(RoseauLoadFlowException) as e:
...
def test_transformer_single_phases():
    ...
    # Not allowed
    tp = TransformerParameters.from_open_and_short_circuit_tests(
        id="160kVA", type="single", up=20000, us=400, sn=160 * 1e3, p0=460, i0=2.3 / 100, psc=2350, vsc=4 / 100
    )
    for ph in ("ba", "nc", "anb", "nabc", "acb"):
        with pytest.raises(RoseauLoadFlowException) as e:
            Transformer(id="tr1", bus1=bus1, bus2=bus2, phases1=ph, phases2=ph, parameters=tp)
...
def test_transformer_center_phases():
    ...
    # Not allowed
    tp = TransformerParameters.from_open_and_short_circuit_tests(
        id="160kVA", type="center", up=20000, us=400, sn=160 * 1e3, p0=460, i0=2.3 / 100, psc=2350, vsc=4 / 100
    )
    for ph in ("ba", "nc", "anb", "nabc", "acb"):
        with pytest.raises(RoseauLoadFlowException) as e:
            Transformer(id="tr1", bus1=bus1, bus2=bus2, phases1=ph, phases2=ph, parameters=tp)
