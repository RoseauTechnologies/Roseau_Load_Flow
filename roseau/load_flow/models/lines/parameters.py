...
class LineParameters(Identifiable, JsonMixin, CatalogueMixin[pd.DataFrame]):
    ...
    def __init__(
        ...
        self._check_matrix_shape(id=id, matrix=self._z_line, matrix_name="z_line")
        if y_shunt is None:
            self._y_shunt = np.zeros_like(self._z_line, dtype=np.complex128)
            self._y = np.zeros_like(self._z_line, dtype=np.complex128)
        else:
            self._y_shunt = np.array(y_shunt, dtype=np.complex128)
            self._check_matrix_shape(id=id, matrix=self._y_shunt, matrix_name="y_shunt")
    ...
    @ureg_wraps("ohm/km", (None, None))
    def z_line(self, phases: str) -> Q_[ComplexArray]:
        """Retrieve the impedance matrix of the line.

        Args:
            phases:
                The phases of the line to model.
    ...
    def y_shunt(self, phases: str) -> Q_[ComplexArray]:
        """Retrieve the shunt admittance matrix of the line.
    ...
    def with_shunt(self, phases: str) -> bool:
        """`True` if the shunt admittance matrix is not null for the given line's phases.
    ...
    def _sym_to_zy_simple(z0: complex, z1: complex, y0: complex, y1: complex) -> tuple[ComplexArray, ComplexArray]:
        ...
        z = np.full((4, 4), fill_value=zm, dtype=np.complex128)
        y = np.full((4, 4), fill_value=ym, dtype=np.complex128)
    ...
    @classmethod
    def from_coiffier_model(cls, name: str, id: Id | None = None) -> Self:  # noqa: C901
        ...
        z_line = (r + x * 1j) * np.eye(4, dtype=np.float64)  # in ohms/km
        y_shunt = b * 1j * np.eye(4, dtype=np.float64)  # in siemens/km
    ...
    def from_power_factory(  # noqa: C901
        ...
        *,
        ...
        z_line, y_shunt = cls._sym_to_zy_simple(z0=r0 + 1j * x0, z1=r1 + 1j * x1, y0=1j * b0 * 1e-6, y1=1j * b1 * 1e-6)
        neutral_isna = pd.isna([rn, xn, bn, rpn, xpn, bpn])
        neutral_allna = neutral_isna.all()
        neutral_anyna = neutral_isna.any()
        if not neutral_allna:
            if neutral_anyna:
                msg = f"rn, xn, bn, rpn, xpn and bpn must be all provided or none of them for line parameters {id!r}."
                logger.error(msg)
                raise RoseauLoadFlowException(msg=msg, code=RoseauLoadFlowExceptionCode.BAD_LINE_MODEL)
            else:
                zn = rn + 1j * xn
    ...
    def from_open_dss(
        ...
        *,
        ...
        z, yc = cls._sym_to_zy_simple(z0=z0, y0=yc0, z1=z1, y1=yc1)
    ...
    @classmethod
    @ureg_wraps(None, (None, None, None, None, None, None, None, "mm**2", "mm**2", None))
    def from_catalogue(
        cls,
        ...
        z_line = (r + x * 1j) * np.eye(4, dtype=np.complex128)
        z_line[-1, -1] = rn + 1j * xn
        y_shunt = (b * 1j) * np.eye(4, dtype=np.complex128)
        y_shunt[-1, -1] = bn * 1j
        ampacities = [ampacity] * 3 + [ampacity_neutral]
        materials = [material] * 3 + [material_neutral]
        insulators = [insulator] * 3 + [insulator_neutral]
        sections = [section] * 3 + [section_neutral]
        if id is None:
            id = name
        return cls(
            id=id,
    ...
    def _to_dict(self, include_results: bool) -> JsonDict:
        ...
        if not np.allclose(self._y_shunt, 0):
    ...
    #
    @staticmethod
    def _check_matrix_shape(id: Id, matrix: ComplexArray, matrix_name: Literal["z_line", "y_shunt"]) -> None:
        """Check a matrix shape. It is used in the constructor to ensure 4x4 matrices."""
        if matrix.shape != (4, 4):
            msg = (
                f"The {matrix_name} matrix of line type {id!r} has incorrect dimensions {matrix.shape}. A "
                f"4x4 matrix was expected."
            )
            logger.error(msg)
            raise RoseauLoadFlowException(msg=msg, code=RoseauLoadFlowExceptionCode[f"BAD_{matrix_name.upper()}_SHAPE"])

    @staticmethod
    def _check_matrix_values(id: Id, matrix: ComplexArray, matrix_name: Literal["z_line", "y_shunt"]) -> None:
        """Check the coefficients of a matrix"""
        ...
        if not np.allclose(off_diagonal_elements.real, 0):
            msg = f"The {matrix_name} matrix of line type {id!r} has off-diagonal elements with a non-zero real part."
        ...
        if (matrix.real < 0.0).any():
            msg = f"The {matrix_name} matrix of line type {id!r} has coefficients with negative real part."
            logger.error(msg)
            raise RoseauLoadFlowException(msg=msg, code=RoseauLoadFlowExceptionCode[f"BAD_{matrix_name.upper()}_VALUE"])

    @staticmethod
    def _check_z_line_matrix(id: Id, z_line: ComplexArray) -> None:
        """Check that the z_line matrix is not singular."""
        if nplin.det(z_line) == 0:
            msg = f"The line type {id!r} produces invalid line impedance matrix."
            logger.error(msg)
            raise RoseauLoadFlowException(msg=msg, code=RoseauLoadFlowExceptionCode.BAD_Z_LINE_VALUE)

    @classmethod
    def _convert_y(cls, y: ComplexArray) -> ComplexArray:
        """Convert an *admittance* matrix into a *shunt admittance* matrix or a *shunt admittance* matrix into an
        *admittance* matrix. The transformation is symmetrical.
        ...
        Returns:
            The shunt admittance matrix if an admittance matrix was provided or an admittance matrix if a shunt
            admittance was provided.
        """
        y_converted = -(y.copy())
        np.fill_diagonal(y_converted, np.einsum("ij->i", y))
        return y_converted
    ...
    def _check_enum_array(
        ...
        if np.isscalar(value_isna):
            return None if value_isna else np.array([enum_class(value) for _ in range(4)], dtype=np.object_)
            ...
            if len(value) != 4:
                msg = f"Incorrect number of {name}: {len(value)} instead of 4."
                logger.error(msg)
    ...
    def _check_positive_float_array(
        value: FloatArray | Sequence[Float] | Float | None, name: Literal["sections", "ampacities"], unit: str
    ) -> FloatArray | None:
        value_isna = pd.isna(value)
        if np.isscalar(value_isna):
            if value_isna:
                ...
                return np.array([value for _ in range(4)], dtype=np.float64)
        else:
